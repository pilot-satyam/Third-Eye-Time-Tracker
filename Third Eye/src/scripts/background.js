...
function checkDOM(state, activeUrl, tab, activeTab) {
    if (state === 'idle' && isDomainEquals(activeUrl, "youtube.com")) {
        ...
        else if (state === 'idle' && isDomainEquals(activeUrl, "primevideo.com")) {
            trackForAmazonPrime(mainTRacker, activeUrl, tab, activeTab);
    } else activity.closeIntervalForCurrentTab();
}
...
function trackForAmazonPrime(callback, activeUrl, tab, activeTab) {
    if (isHasPermissioForAmazonPrime) {
        executeScriptAmazonPrime(callback, activeUrl, tab, activeTab);
    } else {
        checkPermissionsForAmazonPrime(executeScriptAmazonPrime, activity.closeIntervalForCurrentTab, callback, activeUrl, tab, activeTab);
    }
}
...
function executeScriptAmazonPrime(callback, activeUrl, tab, activeTab) {
    chrome.tabs.executeScript({ code: "var videoElement = document.getElementsByTagName('video')[0]; (videoElement !== undefined && videoElement.currentTime > 0 && !videoElement.paused && !videoElement.ended && videoElement.readyState > 2);" }, (results) => {
        if (results !== undefined && results[0] !== undefined && results[0] === true) {
            callback(activeUrl, tab, activeTab);
        } else {
...
function loadPermissions() {
    ...
    checkPermissionsForAmazonPrime();
...
function checkPermissionsForAmazonPrime(callbackIfTrue, callbackIfFalse, ...props) {
    chrome.permissions.contains({
        permissions: ['tabs'],
        origins: ["https://www.primevideo.com/*"]
    }, function(result) {
        if (callbackIfTrue != undefined && result)
            callbackIfTrue(...props);
        if (callbackIfFalse != undefined && !result)
            callbackIfFalse();
        isHasPermissioForAmazonPrime = result;
    });
}
